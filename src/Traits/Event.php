	...
	protected function getEventListener($listener)
	{
		if(is_callable($listener) || ($listener instanceof \Closure))
		{
			...
			try
			{
				...
				if($method->isStatic())
				{
					throw new Exception(setcooki_sprintf("listener method: %s is static", $listener));
				}
				if($method->isAbstract())
				{
					throw new Exception(setcooki_sprintf("listener method: %s is abstract", $listener));
				}
				if(!$method->isPublic())
				{
					throw new Exception(setcooki_sprintf("listener method: %s is not public", $listener));
				}
				return  array($this, $listener);
			}
			catch(\ReflectionException $e)
			{
				throw new Exception(setcooki_sprintf("listener method: %s does not exist or is not accessible: %s", array($listener, $e->getMessage())));
			}
		}
	}
}